 ...
 * @async
 *
 * @param {string} tableName - Table name as defined in the schema.
 ...
 * @param {string} [prefixAddress=EbakusDBClient.address] - The address defined for this EbakusDBClient.
 *
 * @throws {Error}
...
EbakusDBClient.prototype.get = async function get(
  ...
  orderClause,
  blockNumber = 'latest',
  prefixAddress = this.address
) {
  if (!this.provider.utils.isHexStrict(blockNumber)) {
    blockNumber = this.provider.utils.toHex(blockNumber)
  }
